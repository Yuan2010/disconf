...
public class ScanDynamicStoreAdapter {
    ...
    private static ScanDynamicModel analysis4DisconfUpdate(ScanStaticModel scanModel, Registry registry) {
        ...
        if (scanModel.getiDisconfUpdatePipeline() != null) {
            IDisconfUpdatePipeline iDisconfUpdatePipeline =
                    getIDisconfUpdatePipelineInstance(scanModel.getiDisconfUpdatePipeline(), registry);
    ...
    private static void transformPipelineService(IDisconfUpdatePipeline iDisconfUpdatePipeline) {
        DisconfStorePipelineProcessor disconfStorePipelineProcessor =
                DisconfStoreProcessorFactory.getDisconfStorePipelineProcessor();
    ...
    private static void transformUpdateService(Map<DisconfKey, List<IDisconfUpdate>> disconfUpdateServiceInverseIndexMap) {
        DisconfStoreProcessor processorFile = DisconfStoreProcessorFactory.getDisconfStoreFileProcessor();
        DisconfStoreProcessor processorItem = DisconfStoreProcessorFactory.getDisconfStoreItemProcessor();
        for (DisconfKey disconfKey : disconfUpdateServiceInverseIndexMap.keySet()) {
            try {
                if (disconfKey.getDisConfigTypeEnum().equals(DisConfigTypeEnum.FILE)) {
                    if (!processorFile.hasThisConf(disconfKey.getKey())) {
                        throw new Exception();
                    }
                    processorFile.addUpdateCallbackList(disconfKey.getKey(), disconfUpdateServiceInverseIndexMap.get(disconfKey));
                } else if (disconfKey.getDisConfigTypeEnum().equals(DisConfigTypeEnum.ITEM)) {
                    if (!processorItem.hasThisConf(disconfKey.getKey())) {
                        throw new Exception();
                    }
                    processorItem.addUpdateCallbackList(disconfKey.getKey(), disconfUpdateServiceInverseIndexMap.get(disconfKey));
                }
            } catch (Exception e) {
