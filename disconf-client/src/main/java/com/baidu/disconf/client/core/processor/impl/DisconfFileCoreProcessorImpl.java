...
public class DisconfFileCoreProcessorImpl implements DisconfCoreProcessor {
    ...
    private WatchMgr watchMgr = null;      // 监控器
    private FetcherMgr fetcherMgr = null;  // 抓取器
    // 仓库算子
    private DisconfStoreProcessor disconfStoreProcessor = DisconfStoreProcessorFactory.getDisconfStoreFileProcessor();
    private Registry registry = null;      // bean registry
    ...
    private void updateOneConfFile(String fileName, DisconfCenterFile disconfCenterFile) throws Exception {
        ...
        if (DisClientConfig.getInstance().ENABLE_DISCONF) {
            ...
            try {
                ...
                filePath = fileName;  // change file path
            }
    ...
    private void callUpdatePipeline(String key) {
        ...
        if (object != null) {
            ...
            IDisconfUpdatePipeline updatePipeline = DisconfCenterStore.getInstance().getiDisconfUpdatePipeline();
            if (updatePipeline != null) {
                try {
                    updatePipeline.reloadDisconfFile(key, disconfCenterFile.getFilePath());
                } catch (Exception e) {
                    LOGGER.error(e.toString(), e);
                }
            }
    ...
    private void updateOneConf(String fileName) throws Exception {
        ...
        if (disconfCenterFile != null) {
            updateOneConfFile(fileName, disconfCenterFile);  // 更新仓库
            inject2OneConf(fileName, disconfCenterFile);     // 更新实例
        }
    }
